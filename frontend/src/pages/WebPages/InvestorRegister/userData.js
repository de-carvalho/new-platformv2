...
export default function InvestorRegister() {
  ...
  function handleToStepTwo(event) {
    ...
    if (
      ...
      data.genre &&
      data.breed) {
        localStorage.setItem('investorOne', JSON.stringify(data))
      history.push('/investorRegister/stepTwo')
    }
  }

  useEffect(() => {
    if(localStorage.getItem('investorOne')) {
      let json = JSON.parse(localStorage.getItem('investorOne'))
      setName(json.name)
    ...
    <div id="investorOne">
      ...
      <div className="mainInvestOne">
        ...
        <form>
            ...
            <button className="btn mx-auto" type="submit" onClick={handleToStepTwo}>
              Continuar
            </button>
          </form>
        </div>
