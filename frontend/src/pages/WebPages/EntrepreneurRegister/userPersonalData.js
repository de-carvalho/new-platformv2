...
export default function EntrepreneurRegisterTwo() {
  ...
  function handleToStepThree(event) {
    ...
    if (
      ...
      data.state &&
      data.neighborhood) {
      localStorage.setItem('entrepreneurTwo', JSON.stringify(data))
      history.push('/entrepreneurRegister/stepThree')
    }
  }

  useEffect(() => {
    if(localStorage.getItem('entrepreneurTwo')) {
      let json = JSON.parse(localStorage.getItem('entrepreneurTwo'))
      setPhone(json.phone)
      setCep(json.cep)
  ...
  return (
    <div id="stepTwo">
      ...
      <div className="mainTwo">
        <h3>Cadastro de entrepreneur - 2/5</h3>
