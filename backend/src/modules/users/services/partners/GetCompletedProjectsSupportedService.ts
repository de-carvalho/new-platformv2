...
@injectable()
class GetProjectsSupportedCompleted {
  constructor(
  ...
  public async execute(userId: number): Promise<Project[] | undefined> {
    const findProjects = await this.usersRepository.getProjectsSupportedCompleted(
      userId,
    );
...
export default GetProjectsSupportedCompleted;
